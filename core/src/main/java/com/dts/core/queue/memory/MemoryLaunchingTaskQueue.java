package com.dts.core.queue.memory;

import com.dts.core.TaskInfo;
import com.dts.core.queue.LaunchingTaskQueue;
import com.dts.rpc.DTSConf;
import com.google.common.collect.ArrayListMultimap;
import com.google.common.collect.ListMultimap;
import com.google.common.collect.Maps;

import java.util.List;
import java.util.Map;

/**
 * @author zhangxin
 */
public class MemoryLaunchingTaskQueue implements LaunchingTaskQueue {

  private final Map<String, TaskInfo> launchingTasks = Maps.newLinkedHashMap();
  private final ListMultimap<String, TaskInfo> taskIdToTasks = ArrayListMultimap.create();
  private final DTSConf conf;

  public MemoryLaunchingTaskQueue(DTSConf conf) {
    this.conf = conf;
  }

  @Override public boolean add(TaskInfo task) {
    TaskInfo oldTask = launchingTasks.put(task.getId(), task);
    if (oldTask == null) {
      taskIdToTasks.put(task.getId(), task);
    }
    return oldTask == null;
  }

  @Override public boolean remove(String id) {
    TaskInfo oldTask = launchingTasks.remove(id);
    return oldTask != null;
  }

  public List<TaskInfo> getByTaskId(String taskId) {
    return taskIdToTasks.get(taskId);
  }
}
