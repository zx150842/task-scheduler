...
import com.dts.core.exception.DTSSerializeException;
...
public class NettyRpcEnvTest {
  ...
  private Tuple2 setupNetworkEndpoint(RpcEnv _env) {
    ...
    RpcEndpointRef endpointRef = _env.setupEndpoint("network-events-non-client", new RpcEndpoint(_env) {
      ...
      @Override public void onConnected(RpcAddress remoteAddress) {
        events.add(new Tuple2("onConnected", remoteAddress));
      }

      @Override public void onDisconnected(RpcAddress remoteAddress) {
        events.add(new Tuple2("onDisconnected", remoteAddress));
      }

      @Override public void onNetworkError(Throwable cause, RpcAddress remoteAddress) {
        events.add(new Tuple2("onNetworkError", remoteAddress));
      }
    });
    return new Tuple2<>(endpointRef, events);
  }
  ...
  public void testNetworkEventsInServerEnvWhenAnotherEnvInServerMode() throws InterruptedException {
    ...
    Tuple2 pair1 = setupNetworkEndpoint(serverEnv1);
    ConcurrentLinkedQueue events = (ConcurrentLinkedQueue) pair1._2;
    Tuple2 pair2 = setupNetworkEndpoint(serverEnv2);
    RpcEndpointRef serverRef2 = (RpcEndpointRef) pair2._1;
    try {
      ...
      TimeUnit.SECONDS.sleep(2);
      assertTrue(events.contains(new Tuple2<>("onConnected", serverEnv2.address())));
      ...
      TimeUnit.SECONDS.sleep(2);
      assertTrue(events.contains(new Tuple2<>("onConnected", serverEnv2.address())));
      assertTrue(events.contains(new Tuple2<>("onDisconnected", serverEnv2.address())));
    } finally {
      serverEnv1.shutdown();
      serverEnv2.shutdown();
  ...
  public void testNetworkEventInServerEnvWhenAnotherEnvInClientMode() throws InterruptedException {
    ...
    Tuple2 pair = setupNetworkEndpoint(serverEnv);
    RpcEndpointRef serverRef = (RpcEndpointRef) pair._1;
    ConcurrentLinkedQueue<Tuple2> events = (ConcurrentLinkedQueue) pair._2;
    ...
    try {
      ...
      for (Tuple2 event : events) {
        if (event._1.equals("onConnected")) {
      ...
      for (Tuple2 event : events) {
        if (event._1.equals("onConnected")) {
          onConnected = "onConnected";
        } else if (event._1.equals("onDisconnected")) {
  ...
  public void testNetworkEventInClientEnvWhenAnotherEnvInServerMode() throws InterruptedException {
    RpcEnv clientEnv = createRpcEnv(new DTSConf(false), "client", 0, true);
    RpcEnv serverEnv = createRpcEnv(new DTSConf(false), "server", 0, false);
    Tuple2 pair1 = setupNetworkEndpoint(clientEnv);
    ConcurrentLinkedQueue<Tuple2> events = (ConcurrentLinkedQueue<Tuple2>) pair1._2;
    Tuple2 pair2 = setupNetworkEndpoint(serverEnv);
    RpcEndpointRef serverRef = (RpcEndpointRef) pair2._1;
    try {
      RpcEndpointRef serverRefInClient = clientEnv.setupEndpointRef(serverRef.address(), serverRef.name());
      serverRefInClient.send("hello");
      TimeUnit.SECONDS.sleep(2);
      assertTrue(events.contains(new Tuple2("onConnected", serverEnv.address())));
      ...
      TimeUnit.SECONDS.sleep(2);
      assertTrue(events.contains(new Tuple2("onConnected", serverEnv.address())));
      assertTrue(events.contains(new Tuple2("onDisconnected", serverEnv.address())));
    } finally {
      clientEnv.shutdown();
      serverEnv.shutdown();
      clientEnv.awaitTermination();
